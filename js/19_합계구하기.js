...
addBtn.addEventListener("click", () => {
  ...
  input.type = "text"; // input 타입 추가
  input.classList = "input-number";

  // 3) span 만들기
  const span = document.createElement("span");
  span.className = "remove-row"; // span 클래스 추가
  span.innerHTML = "&times;"; // HTML 특수문자는 innerHTML 이용
  // ----------제거하기------------
  // x 버튼(span)이 만들어 질 때 click되었을 때 동작을 같이 정의
...
// 페이지 로딩 시점에 존재하는 .remove-row를 모두 얻어오기 -> 처음에 x버튼이 없어서 아래 for문 실행 X
