...
// 선언된 변수는 콘솔에서도 사용가능
// (에러)number3 is not defined : number3라는 변수가 정의되지 않았다 == number3 선언 안됨 / 존재X

/* 1. var : 변수, 변수명 중복 허용(덮어쓰기), 함수 레벨 범위(scope)*/
function varCheck(){
  var menu = "콩국수";
  ...
  var menu = "국밥";
  console.log("변경된 menu : ", menu);
  // var를 이용할 경우 변수 명이 중복되면 이전 값을 새로운 값으로 덮어쓰기함.
  // 문제점 : 이전에 대입한 값을 이용할 수 없다.
  // 나중에 이전 값을 사용하고 싶으나 나도 모르게 새로운 값을 대입해 지워버리는 경우가 생김.
}

/* 2. let : 변수, 변수명 중복 비허용, 블록 {} 레벨 범위 */
function letCheck(){
  let name = "홍길동";
 ...
 /* let */ name = "김미영" // 주석 해제하면 오류 발생.
  ...
  // 1. 해결 방법 : 변수에 대입된 값을 바꾸는게 목적 -> let 제거 : name = "김미영";
  // 2. 해결방법 : 새로운 변수 생성이 목적 -> 변수명 변경 -> let name2 = "김미영";
...
/* 3. const : 상수(constant, 변하지 않는 수, 항상 같은 수), 변수명 중복 비허용, 블록 {} 레벨 범위 */
// const로 선언된 변수에 값을 대입하면 변경할 수 없다(새로운 값 대입 불가)
function constCheck(){
  ...
  // Uncaught TypeError: Assignment to constant variable. : 상수 변수에 값이 할당됨 -> 상수는 값을 다시 대입(할당) 할 수 없어서 오류 발생
...
2. 지역 변수 : 특정 지역에서 사용 가능한 변수
  1) 함수 레벨 범위 : function 함수명(){}
  - 함수 정의 부분에서 {} 내부에 var로 선언된 변수는 함수 내부 어디서든 사용 가능
  2) 블록 레벨 범위 : 
 */
// 함수 선언
function scopeTest(){
...
function scopeTest2(){
  ...
  // const temp = 234; // [변수명 중복] 블록 범위 변수 'temp'를 다시 선언할 수 없습니다.
  ...
  if(1 == 1){ // 1과 1이 같다면 if {} 수행
    ...
    fn2 = "변경된 fn2";
    // if {} 내부에서 함수 {} 내부(넓은 범위)의 변수 값을 변경
  ...
  console.log("if 실행 후 fn : ", fn2); // 변경된 fn2
...
function typeCheck(){
  ...
  /* undefind : 정의되지 않은 병수 / 값 대입 X 또는 애초에 존재하지 않은 변수 */
  ...
  /* object(객체) : (쉬운 버전  설명) 값을 여러개 저장할 수 있는 형태 */
  // 1) 배열 : 변수의 묶음
  ...
  /* 2) JS 객체 */
  // 값을 K:V 형식으로 여러개 저장하는 형태 -> {K:V, K:V, K:V}
  // 쉽제 이해하는 방법 : K(key) == 변수명, V(value) == 변수에 저장된 값
...
함수명을 잘라서 const에 덮어씌운뒤 값이 function이 된다
const 함수명 = function(){} */
const sumFn= function(a, b){ return a + b; }
console.log( typeof sumFn ); // function
console.log( sumFn ); // F(a, b){return a + b}
