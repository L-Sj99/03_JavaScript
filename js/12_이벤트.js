...
function check1(btn){
  ...
  console.log(btn);
  // 클릭된 요소(btn)에 작성된 내용이 숫자이면 1증가 아니면 0을 대입
  // NaN : 숫자 아님, isNaN(값) : 숫자 아니면 true, Number(값) : number 타입으로 변환
  //을 알아야 요구되는 코드를 쓸 수 있다
  // Number(btn,innerText) === NaN
  if( isNaN(Number(btn.innerText))){ // 숫자가 아니면
    btn.innerText = 0;
  } else{
    // ++ 증감 연산자는 오로지 number 타입에만 사용 가능
    // -> 피연산자가 숫자 형태의 문자열인 경우("123") 강제로 숫자 타입으로 바꿔서 연산을 수행
    btn.innerText++;
...
// function(){} : 익명 함수(이름이 없는 함수) -> 보통 이벤트 핸들러 작성 시 많이 사용됨
test1a.onclick = function(){
  // 고전 이벤트 모델에 있는 이벤트 핸들러 내에서 this : 클릭된 요소
  console.log(this);
  alert("확인 버튼 클릭됨");
}
...
test1b.onclick = function(){
  test1a.onclick = null; // null : 참조하는게 없다 -> 유사 의미 : 없다
  alert("확인 버튼 클릭 이벤트 제거");
}

/* 고전 이벤트 모델 단점 확인 */
const test1c = document.querySelector("#test1c");
// DOM 요소는 onclick, onchange 같은 이벤트 리스너가 필드(변수)로 하나씩 존재함 -> 변수는 마지막으로 대입된 값으로 덮어 씌워진다는 특징이 있다
// 첫 번째 대입
test1c.onclick = function(){
  test1c.style.backgroundColor = "blue";
};
// 두 번째 대입
test1c.onclick = function(){
...
/* #test2 클릭 시 숫자 증가하는 이벤트(리스너, 핸들러 다 묶어서 이야기) 추가 */
test2.addEventListener("click", function(){
  // test2 요소에 내용을 1증가
  test2.innerText++;
});
...
// -> 버튼을 무꺼서 저장한 배열 btns는 요소가 아니다
// -> 요소.addEventListner()는 요소한테만 사용 가능하다
