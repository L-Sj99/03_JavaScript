...
for(let i = 0 ; i < btns.length ; i++){
  btns[i].addEventListener("click", () => {
    ...
    switch(str){
      ...
      case "before" : 
        const div1 = document.createElement("div"); // div요소 생성
        div1.innerText = count; // 내용으로 count 추가
        div1.style.backgroundColor = "greenyellow"; // 배경색 추가
        div1.className = "box"; // box 클래스 추가
        standard.before(div1); // #strandard 앞쪽에 추가
      break;

      case "after" :
        const div2 = document.createElement("div");
        div2.innerText = count;
        div2.style.backgroundColor = "yellowgreen";
        div2.className = "box";
        standard.after(div2); // #strandard 뒤쪽에 추가
      break;
      
      case "remove" :
        document.querySelector(".container").remove(); // .container 요소를 제거
      break;
    }
    count++; // count 1 증가
  });
