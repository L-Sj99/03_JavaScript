...
document.querySelector("#btn3").addEventListener("click", function(){
  ...
  // 2) inputs 각 index 요소의 값을 얻어오기
  for(let i = 0 ; i < inputs.length ; i++){
    const num = Number(inputs[i].value);

    // 3) numberArr의 i번째 index 요소에 num을 저장
...
// -> 해당 이벤트 전용 기능인 경우가 많아서(다른 곳에서는 재사용을 못하는 경우가 많아서)
btn6.addEventListener("click", function(){ // 익명 함수
  ...
  // 변수명이 익명 함수의 함수명 역할을 해준다(요즘 함수 정의 방법 + function 단어 안쓰기가 트렌드)
  // 해당 함수는 지역 변수에 저장됨 -> 해당 지역에서만 사용 가능한 함수
...
// 이름 있는 함수 선언/정의 -> 여러 곳에서 재사용이 가능하다는 특징이 있다
function testFn6(){
...
function testFn7(otherFn){ // otherFn : 전달 받은 함수 코드
  return function(num){
    // otherFn() : 전달 받은 함수 호철(실행)
    return otherFn() + num;
  }
}
...
arrows[2].addEventListener("click", () => {
  ...
  const minusFn = (a, b) => a - b; // {return이 생략된 경우}
  alert(minusFn(10, 3));
  
  //매개변수 1개, 정의 내용 return 한 줄
  const doubleFn = num =>  num * 2; // ()랑 {return}이 지워진 상태
  console.log(doubleFn(3));
})

// object를 retur하는 경우
// JS 객체 : {k:v, k:v, ...}
// k(key) == 변수명(객체), v(value) == 대입되는 값
