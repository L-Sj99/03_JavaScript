...
/* 배열명.lenght : 배열의 길이 */
  ...
  // 2. 원하는 index에 값을 마음대로 추가할 수 있다 -> 중간에 건너뛴 index 요소는 빈칸
  arr1[5] = '중간에 건너뜀';
...
function check3(){
  ...
  for(let i = 0; i < 4 ; i++){// 0 1 2 3 
    arr[i] = prompt(`${i}번째 index 요소 값 입력`);
  }
  console.log(arr);
}
...
/* 로또 번호 생성기 */
/* 1 ~ 45사이 난수 6개 생성 단, 중복X, 오름 차순 정렬 */
function check6(){
  const lotto = []; // 난수 저장할 배열
  for(let i = 0 ; i < 6 ; i++){
    //1 ~ 45사이 난수
    const rendomNumber = Math.floor(Math.random() * 45 ) + 1 ;
    // 일단 배열에 난수 대입
    lotto[i] = rendomNumber;
    //앞에 중복되는 숫자 있는지 확인
    for(let x = 0 ; x < i ; x++){
      // 중복 검사, i번재 저장하려는 값이 x번째(앞쪽)에 존재 하는가?
      if( lotto[x] === lotto[i]){ // 중복 값이 존재한다
        // 있으면 현재 index에 머물기
        // 난수를 저장하기 위한 index를 지정하는 i 값을 1감소 -> 바깥쪽 for문의 증감 i++에 의해 다시 1 증가
        // 결과적으로 i값 변화가 없는 상태
        i--;
        break; // 불필요한 중복 검사 멈춤(효율 증가)
      }
    }
    //없으면 다음 index로 이동
  }
...
function selectMenu(){
  //점심 메뉴로 초기화된 배열 생성
  const menus = ["라면 + 김밥", "제육볶음", "돈까스", "볶음밥", "우동", "돼지국밥", "불고기", "샌드위치", "우육면", "냉면", "만둣국", "쭈꾸미", "찜닭", "쌀국수", "김치찌개", "육개장"];
...
function check7(){
  ...
  console.log("2차원 배열 arr의 길이(행 개수) : ", arr.length);
  console.log("arr의 0번째 index 길이 : ", arr[0].length);
  console.log("arr의 1번째 index 길이 : ", arr[1].length);
  console.log("arr의 2번째 index 길이 : ", arr[2].length);
  ...
  for(let i = 0 ; i < arr.length ; i++){
    ...
    for(let x = 0 ; x < arr[i].length ; x++){
      // arr[i].length : i번쨰 요소(배열)의 길이 -> 4,4,2 순서
      str += `(${arr[i][x]})`;
    }
...
/* 2차원 배열 확인 2 */
function check8(){
  // 4행 4열짜리 2차원 배열에 0 ~ 99사이 난수 배치
  // 배열 생성
  const arr = []; // [[], [], [], []]로 작성해도 된다, 대신 행을 따로 생성 해야함.
  // 난수를 2차원 배열에 대입
