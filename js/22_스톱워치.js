...
let currentInterval; // START 버튼을 눌었을 때 반복되는 setInterval()을 저장할 용도의 변수

/* start 버튼 클릭 시 100ms 마다 count 1 증가+ 화면 시간 변경 */
startBtn.addEventListener("click", () => {
  // 이 전에 start 버튼이 클릭된 적이 있을 경우 -> 버튼에 작성된 내용이 "PAUSE"인 경우
  if(startBtn.innerText === "PAUSE"){
    clearInterval(currentInterval); // 이전 interval 멈춤
    startBtn.innerText = "START"; // 버튼 내용을 START로 변경

    recordBtn.disabled = true; // 비활성화 o 

    return; // 일시 정지 상태가 유지되어야 하기 때문에 이벤트 핸들러를 여기서 종료 시켜 아래 setInterval()을 수행 못하게 함
  }
  ...
  recordBtn.disabled = false; // 비활성화 x -> 활성화 o 
});
...
/* RECORD 버튼 클릭 시 현재 출력되는 시간을 얻어와 li 태그로 만들어 #recordConrainer의 첫 번째 자식으로 추가 */
