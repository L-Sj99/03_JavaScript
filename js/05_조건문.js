...
function check1() {
  ...
  // 입력 받은 값이 0보다 작서나 같은 경우 == 양수가 아닌 경우
  if (value <= 0) {
...
function check2() {
  ...
  // 소수점 내림처리 : Math.floor(실수) -> 소수점 첫째 자리에서 내림처리 -> 정수형태로 변환
...
function ageCheck2() {
  ...
  let result;
  // 0 미만, 120 초과
  if (0 > value || value > 120) result = "잘못 입력 하셨습니다.";
...
/* if ~ else if ~ else 에서 앞선 if 조건이 false가 되면 해당 범위는 이후 조건에서 제외된다.
  if, else if, if, for, while 등 {}내부에 코드가 한줄만 존재하는 경우 {} 생각 가능하다. */
...
/* Early Return : 두 수 A, B를 입력 받아 A가 B의 배수가 맞는지 확인 */
function check5() {
  const a = document.getElementById("input5a")
  const b = document.getElementById("input5b")

  // A와 B중 하나라도 입력되지 않으면 -> 입력되지 않은 값이 존재합니다
  if (a.value.length === 0 || b.value.length === 0) {
    alert("입력되지 않은 값이 존재합니다");
    return; // Early Reuten : 함수를 여기서 일찍 끝냄
  }
  ...
  // 위 조건이 모드 false이면 배수 판별 진행 -> 배수 맞음 / 배수 아님
  if (valueA % valueB === 0) {
    alert("배수 맞음");
    return;
  }
...
//  매개변수(paramter) : 함수 호출 시 전달된 값(전달 인자, Arguments)을 저장하는 변수
/** switch 버전 계산기 만들기
* @param op : 전달 받은 기호
 */
function calc(op){
...
-> break 미 작성시 현재 case 수행 후 멈추지 않고 break가 존재하는 case문 까지 계속 수행
...
switch(op){
  ...
  // 알맞은 case가 없으면 defailt 실행
  default: result = "잘못된 연산자";
}
  // 아이디가 "calcResult"인 요소를 얻어와 내용(innerText)로 result를 대입
