...
function classTest(){
  /* 유사 배열이란? 배열처럼 index, length를 가지고 있으나 배열 전용 기능(메서드)를 제공하지 않음 */
  // 클래스가 cls-text인 요소를 모두 얻어와 HTML Collection(유사배열) 형태로 얻어옴
...
/* 태그명으로 요소 접근하기  */
function tagNameTast(){
  /* document.getElementsByTageName("태그명") */
  const tagList = document.getElementsByTagName("li"); // -> 요소 4개짜리 배열 반환
  for(let i = 0 ; i < tagList.length ; i ++){// i == 0, 1, 2, 3
...
function nameTest(){
  ...
  // i == 0, 1, 2, 3, 4, 5 (길이는 6, 마지막 index는 5(길이 -1))
  ...
  // const div = document.getElementsByClassName("name-div") -> 요소가 1개밖에 없어도 배열 형태로 반환, s가 들어가면 무조건 배열
  //요소가 1개밖에 없음 === 0번 index만 존재 -> 0번 index만 별도 변수에 저장해서 사용
...
/* #userInput 입력 요소에서 키보드로 값 입력 중 "Enter" 입력이 감지되는 경우
readValue() 함수 호출 */
