...
/*
  -JS 객체 : {K:V, K::V, K:V, ...}
  -*중요* K(Key)는 무조건 string -> "Key" 또는 'Key' 또는 Key
...
btn1.addEventListener("click", () => {
  ...
  // 중복되는 key에 새로운 값 대입 ==> 덮어쓰기
  product.price = 2000; 
  console.log(product);

  // 4. 객체에 새로운 K:V 추가하기
  // -> 객체명.새로운Key = 새로운 Value 또는 객체명["새로운Key"] = 새로운 Value
...
/* 
  ...
  2) <button onclick="testFn(this)">테스트</button> == 여기서 this는 이벤트가 발생한 요소 (클릭된 button)
  3) 화살표 함수 내에서 this == Window 객체(화살표 함수에서는 this를 사용하지 못한다 생각하자)
*/
  ...
  // 메서드는 객체에 종속된(속한) 함수 -> 꼭 객체명.함수명() 형식으로 호출해야한 한다
